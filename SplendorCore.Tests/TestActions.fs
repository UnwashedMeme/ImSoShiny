            ...
            <| fun (turndata:TurnData) (coin:Coin) ->
                ...
                match MainActions.draw2OfSame turndata turndata.currentPlayer coin with
                    ...
                    | Ok ntd -> 
                        ...
                        Expect.equal (ntd.bank.GetCoinCount coin) ((turndata.bank.GetCoinCount coin) - 2) "bank should have 2 less of that coin"
                        Expect.equal (Seq.length player.coins) ((Seq.length turndata.currentPlayer.coins) + 2) "Player should have 2 more coins";
            ...
            <| fun (td:TurnData) -> 
                ...
                match MainActions.draw2OfSame td player (Coin Green) with   
                    ...
                    | Ok newTurnData ->
                        ...
                        Expect.equal 
                            (getPlayerIds newTurnData.players)
                            (getPlayerIds td.players) 
                            "Players changed"
        ...
        testPropertyWithConfig config "next phase"
            <| fun (action:Action) (td:TurnData) ->
                match dispatchAction action td with
                    | Error e -> ()
                    | Ok nextTurnData -> 
                        Expect.equal nextTurnData.phase (nextPhase td.phase) 
                            "Should have given the next phase"
    ]